...
log "Submitting gpurec ($PHASE, root=$ROOT) on Saion: part=$SAION_PARTITION gres=$SAION_GRES"
JOB=$(rsh "$SAION_SSH" "
  cd '$SAION_GPUREC_DIR'
  mkdir -p '$OUTDIR'
  sbatch --parsable \
    -p '$SAION_PARTITION' --gres='$SAION_GRES' -c '$SAION_CPUS' --mem='$SAION_MEM' -t '$SAION_TIME' \
    --export=ALL,GPUREC_DIR='$SAION_GPUREC_DIR',WILLIAMS_DIR='$WILLIAMS_DIR_SAION',ROOT='$ROOT',OUT_RATES='$OUT_RATES',RUSE_OUT='$RUSE',FAMILIES='$N',STEPS='$GPUREC_STEPS',DTYPE='$GPUREC_DTYPE',MIN_SPECIES='$GPUREC_MIN_SPECIES',FM_MODE='$GPUREC_FM_MODE',GMODE='$MODE',FAMILY_BATCH_SIZE='$GPUREC_FAMILY_BATCH_SIZE',PY_MODULE='$SAION_PYTHON_MODULE' \
